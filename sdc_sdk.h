...
/**
	Used to retrieve software name and corresponding 
	version information. 
*/
...
return values:  SDCERR_SUCCESS if successful
				...
				SDCERR_FAIL if radio disabled or internal err
...
/*---------------------------------------------------------
	LRD_WF_GetSSID() will pass the characters of the SSID if associated
	parameter:
		ssidStruct *ssid - a valid pointer to a ssidStruct
	return values:
		SDCERR_INVALID_PARAMETER - if parameter is NULL
		SDCERR_SUCCESS - ssid structure is filled in with value and len of ssid
		SDCERR_FAIL - no ssid
	--note: 
		the returned ssid.val need not be a string and could contain null 
		characters.  ssid.len will indicate the length of the ssid
---------------------------------------------------------*/
